...
```{r echo =FALSE}
# Libraries----
pacman::p_load(rio, tidyverse, stm, tidytext, ggwordcloud, 
               sjPlot, DescTools, ggpage,quanteda, patchwork,
               ggrepel)

# Module----
ind_doc <- modules::use("Scripts/Fun/ind_doc.R")
ours <- modules::use("Scripts/Fun/ours.R")

# Data----
mystm <- rio::import("Clean_Data/mystm.rda")
mydfm <- rio::import("Clean_Data/mydfm.rda")
topic_16 <- rio::import("Clean_Data/topic_16.rda")
full_stm <- rio::import("Clean_Data/full_stm.rda")
tidy_gamma <- rio::import("Clean_Data/tidy_gamma.rda")
tidy_beta <- rio::import("Clean_Data/tidy_beta.rda")
frame_prevalence <- rio::import("Clean_Data/frame_prevalence.rda")
mytext <- rio::import("Clean_Data/mytext.rda")
...
1.  **(Security) Securitization framework**
    -   Topics: 2, 3, 4, 13
2.  **(migrant right) migrant rights / migrant rights framework**
    -   Topics: 7, 9, 16
...
tidy_wordcloud %>% 
  ...
  theme_minimal() +
  labs(title = "Security frame wordcloud",
       subtitle = "Highest probability word") +
  scale_color_gradient(low = "darkred", high = "red")
```

### (migrant right) migrant rights / migrant rights framework

```{r}
set.seed(7)
tidy_wordcloud %>% 
  filter(theme == "2.Migrant rights") %>% 
  arrange(-beta) %>% 
  ggplot(aes(label = term, size = beta, color = beta)) +
  geom_text_wordcloud() +
  scale_size_area(max_size = 15) +
  theme_minimal() +
  labs(title = "Migrant rights frame wordcloud",
       subtitle = "Highest probability word") +
...
tidy_wordcloud %>% 
  ...
  scale_size_area(max_size = 15) +
  theme_minimal() +
  labs(title = "Administration frame wordcloud",
       subtitle = "Highest probability word") +
...
tidy_gamma %>% 
  ...
  labs(title = "Distribution of frames on documents",
       subtitle = "Grouped by frames",
       y = "Proportion",
...
frame_author_prevalence %>% 
  ...
  geom_label(data = filter(frame_author_prevalence, gamma > 0.001),
             aes(label = scales::percent(gamma, 0.1)), position = position_stack(vjust = 0.5)) +
  scale_x_continuous(labels = scales::percent) +
  theme(legend.position = "top") +
  labs(title = str_c("Figure ", 2, ": ", "Proportion of frame by document's author"),
       fill = "Frames",
       y = NULL,
       x = "Proportion")
```
...
gini_doc <- ind_doc$get_gini_doc(tidy_gamma)
```
...
res <- ind_doc$viz_ind_doc(gini_doc, most_even_id, 
                           mydfm, mystm, 
                           tidy_beta, mytext, 
                           tidy_gamma, pose = FALSE)

# res$page_graph

(res$wordcloUd_graph | (res$pie_graph / res$page_graph)) +
  plot_annotation(title = str_c("Figure ", 3, ": ", "Frames distribution in the document '",
                                res$param[[2]], "'"),
                  subtitle = "Melilla’s municipality document")
```

```{r echo=FALSE}
# To search document id
base <- 
  mystm$meta %>% 
  mutate(document = row_number()) %>% 
  left_join(gini_doc)

# More even No Name Kitchen document
nnk_id <- 
  base %>% 
  filter(category == "No Name Kitchen") %>% 
  arrange(gini) %>% 
  slice(1) %>% 
  pull(document)

# More even Frontex document
frontex_id <- 
  base %>% 
  filter(category == "Frontex") %>% 
  arrange(gini) %>% 
  slice(1) %>% 
  pull(document)
```
...
```{r}
res_nnk <- ind_doc$viz_ind_doc(gini_doc, nnk_id, 
                           mydfm, mystm, 
                           tidy_beta, mytext, 
                           tidy_gamma, pose = FALSE, 
                           max_size = 40)

(res_nnk$wordcloUd_graph | (res_nnk$pie_graph / res_nnk$page_graph)) +
  plot_annotation(title = str_c("Figure ", 4, ": ", "Frames distribution in the document '",
                                res_nnk$param[[2]], "'"),
                  subtitle = "No Name Kitchen's document")
```


```{r}
res_frontex <- ind_doc$viz_ind_doc(gini_doc, frontex_id, 
                           mydfm, mystm, 
                           tidy_beta, mytext, 
                           tidy_gamma, pose = FALSE)

(res_frontex$wordcloUd_graph | (res_frontex$pie_graph / res_frontex$page_graph)) +
  plot_annotation(title = str_c("Figure ", 5, ": ", "Frames distribution in the document '",
                                res_frontex$param[[2]], "'"),
                  subtitle = "Frontex' document")
```

```{r}
# TODO find a way to put it in table
reg <- ours$lm_stm(~ fyear + category,
            3, topic_16,
            nsims = 25,
            mystm$meta %>% mutate(fyear = factor(year)))
```


```{r}
df_regression <- 
  map_dfc(reg$tables, ours$reg_tab) %>% 
  janitor::clean_names() %>% 
  select(coef = coef_1,
         "1. Security" = estm_2,
         "2. Migrant rights" = estm_4,
         "3. Administration" = estm_6) %>% 
  mutate(type = case_when(
    coef == "(Intercept)" ~ "",
    str_detect(coef, "\\d{4}") ~ "Year\nref: 2021",
    TRUE ~ "Author\nref: Ceuta"
  ), .before = 1)
...
tbl_regression <-
  ...
  flextable::merge_v(1) %>% 
  flextable::theme_box() %>% 
  flextable::merge_h_range(i = c(2, 5), j1 = "type", j2 = "3. Administration") %>% 
  flextable::align_text_col(align = "center") %>% 
  # flextable::align(i = c(2, 5), j = 1:5)
  flextable::set_header_labels(values = list(type = "",
                                             coef = "variable")) %>% 
  flextable::add_header_lines(values = "Relationship between frames and metadata") %>% 
  flextable::add_header_lines(values = "QR decomposition regression") %>% 
  flextable::add_footer_lines(values = "Observations (N = 223)") %>% 
  flextable::add_footer_lines(values = "p<0.05 ** p<0.01 *** p<0.001 ") %>% 
  flextable::autofit()

flextable::save_as_docx(tbl_regression, path = "Results/Tables/QR_regression.docx")
...
+-----------------------+------------------+--------------------+-------------------+
| 16                    | 16               | 2.  migrant right    | Previous topic    |
+-----------------------+------------------+--------------------+-------------------+
